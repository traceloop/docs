...
A Prompt column allows you to define a custom prompt and run it directly on your Playground data. You can compose prompts with messages (system, user, or assistant), insert dataset variables, and configure which model to use. Each row in your dataset will be passed through the prompt, and the model’s response will be stored in the column.
...
### Status Indicators
- **🔄 Running**: Prompt is being executed
- **✅ Completed**: Execution successful
- **❌ Failed**: Execution failed (hover for error details)
- **⚠️ Outdated**: Input variables changed, needs re-execution
...
```
Column 1: "Initial Analysis" → Analyze customer message
Column 2: "Detailed Response" → "Based on this analysis: {{Initial Analysis}}, write response"
Column 3: "Quality Check" → "Review this response: {{Detailed Response}} for quality"
```
...
Column 3: "Response Comparison" → "Compare these responses: {{GPT-4 Response}} vs {{Claude Response}}"
```
