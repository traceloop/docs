...
<ParamField body="selector" type="object[]">
  ...
  | `value` | string | No | The value to match against (for single-value operators like `equals`, `contains`) |
  | `values` | string[] | No | List of values to match against (for `in`, `not_in` operators) |
  | `source` | string | Yes | Where the attribute lives: `span_attributes` or `resource_attributes` |
  | `operator` | string | No | Comparison operator. Defaults to `equals`. One of: `equals`, `not_equals`, `contains`, `not_contains`, `in`, `not_in` |
...
```bash
curl -X POST https://app.traceloop.com/api/v2/auto-monitor-setups \
