...
<Warning>
  ...
  importing any LLM module like OpenAI. However, you can call `initialize()`
  wherever you want.
</Warning>
...
If you're using an LLM framework like Haystack, Langchain or LlamaIndex - we'll do that for you.

Otherwise, you can do it yourself.
We have a set of [decorators](/openllmetry/decorators) to make this easier.
Assume you have a function that renders a prompt and calls an LLM, simply add `@workflow` (or if you have a Python asynchronous method - `@aworkflow`).
...
For more information, see the [dedicated section in the docs](/openllmetry/decorators).
