...
## Traceloop Sync (polling)

By default, the Traceloop SDK's sync functionality is active.
To disable it or change any defaults, see the example below.
...
Traceloop.initialize({ 
  traceloopSyncEnabled: true, // also via TRACELOOP_SYNC_ENABLED env variable (default: true)
  traceloopSyncMaxRetries: 3, // also via TRACELOOP_SYNC_MAX_RETRIES env variable (default: 3)
  traceloopSyncPollingInterval, // also via TRACELOOP_SYNC_POLLING_INTERVAL env variable (default: 60) - seconds
  traceloopSyncDevPollingInterval // also via TRACELOOP_SYNC_DEV_POLLING_INTERVAL env variable (default: 5) - seconds
 });
```
