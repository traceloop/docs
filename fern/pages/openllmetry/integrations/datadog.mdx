...
See the [Datadog documentation](https://docs.datadoghq.com/opentelemetry/) for more information. 

Exporting directly to an agent is easiest. To do that, first enable the OTLP HTTP collector in your agent configuration. This depends on how you deployed your Datadog agent. For example, if you’ve used a Helm chart, you can add the following to your `values.yaml` (see [this](https://docs.datadoghq.com/opentelemetry/otlp_ingest_in_the_agent/?tab=kuberneteshelmvaluesyaml#enabling-otlp-ingestion-on-the-datadog-agent) for other options):
